...
def main():
  ...
  parser.add_argument('src_dir', help='Input text file')
  ...
  while sentenses:
    batched_input = []
    if len(sentenses) <= batch_size:
        batched_input = sentenses.copy()
        sentenses.clear()
    else:
        batched_input = sentenses[:batch_size]
        sentenses = sentenses[batch_size:]
    t = time.time()
    output = translator.translate(batched_input)
    for o in output:
        print(o)
